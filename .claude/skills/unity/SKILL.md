...
**`tests/` runs green on the 6000.5 line, and the note here has been wrong twice.** It first said 6000.5 could not load the suite at all; then that both suites were green; then that PlayMode had 15 permanent failures there. Measured on **6000.5.9f1** with no `UNITY_VERSION`: EditMode **342/350** (8 skipped, zero failures), PlayMode **690/701** (11 skipped, zero failures).

**The 15 failures that used to be here were real, and were fixed by changing the transpiler.** Worth keeping the diagnosis, because the mechanism can bite anywhere. `ButtonTests` and `InputTests` died in `BeforeTest` with an uncatchable `StackOverflowException` across all three engines — and the tests were not what ran out of stack, *Babel* was. `CodeTransformer` is a static singleton built with `JavascriptEngineType.Auto`, so every fixture's JSX transform runs through **QuickJS** whatever engine parameterises the test, which is why the Jint and ClearScript rows reported a QuickJS stack.

quickjs-ng measures its stack limit against the thread that created the runtime — Unity's main thread, already deep in Unity's frames and deeper inside a coroutine. Measured by capping `ScriptRuntime.MaxStackSize`: at 768 KB an overflow raises cleanly as a `RangeError`, at 1 MB (ng's default) it does not raise at all and the real stack dies instead. So the headroom there sits between the two, and Babel wanted more. 6000.1.9f1 gave it that, by a margin — which is why CI never saw this.

The fix was not a bigger stack. Pruning Babel's presets down to the one plugin the suite actually needs changed **nothing**, because the depth is Babel's parse-then-traverse floor rather than the plugins above it. The suite uses **Sucrase** now, which rewrites a token stream and never builds an AST; the 15 failures went to zero with no other change. If you are tempted to put Babel back, that is the measurement to beat.
