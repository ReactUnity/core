...
- A run that leaves `1 failed` with a `TextMesh Pro Essential Resources are missing` message is environmental, not yours. The error surfaces when the TMP importer window closes and attaches to whichever test happens to be running, so it lands on a different test each time.
...
**The `windows/` baselines were regenerated on 2026-07-28** (Unity 6000.1.4f1), because 11 linear-gradient snapshots had gone stale — their pixel-diff counts were byte-identical on 6000.5.5f1 and 6000.1.4f1, so it was drift in the baselines, not a rendering regression. Regenerating rewrote 89 of 191 files: the 11 real failures plus 78 that were passing under the 0.5% threshold with a few stray pixels each, since overwrite mode rewrites on *any* nonzero diff.

`windows/` is therefore tuned to one dev machine's GPU, which is what it is for. `linux/` is the one CI reads and must be regenerated by CI — `[snapshots]` in a commit message, or the workflow's `overwrite-snapshots` input. Never regenerate `linux/` locally.
