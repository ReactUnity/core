...
**`tests/` runs on the 6000.5 line, but PlayMode is not green there** — an earlier note here said it could not load at all, and that was wrong; a later one said both suites were green, and that is wrong too. Measured on **6000.5.9f1** with no `UNITY_VERSION`: EditMode **342/350** (8 skipped, zero failures), PlayMode **675/701** with **15 failures** — `ButtonTests` and `InputTests` across all three engines, each a `StackOverflowException` in `BeforeTest`. Verified pre-existing by stashing every local change and re-running, and CI's 6000.1.9f1 passes the same commit.

**It is a stack-headroom margin, not a bug, and the cause is worth knowing because it can bite anywhere.** Those tests are not running out of stack; *Babel* is. `CodeTransformer` is a static singleton built with `JavascriptEngineType.Auto`, so every fixture's JSX transform runs Babel through **QuickJS** whatever engine parameterises the test — which is also why the Jint and ClearScript rows report a QuickJS stack, and it is not one failure poisoning the rest: run `InputTests` alone and all 9 fail, the first included.

Babel's parser recurses deeper than anything else in the suite, and quickjs-ng measures its stack limit against the thread that created the runtime — Unity's main thread, already deep in Unity's frames and deeper inside a coroutine. Measured by capping `ScriptRuntime.MaxStackSize` and re-running: at 768 KB the overflow raises cleanly as a `RangeError`, at 1 MB (ng's default) it does not raise at all and the real stack dies instead. So the headroom on 6000.5.9f1 sits between the two, and Babel needs more than that. On 6000.1.9f1 it fits, by a margin.

Two consequences. A PlayMode failure in those two fixtures on 6000.5 is the editor, not your change — confirm anything else against a CI editor. And **do not "fix" it by capping `MaxStackSize`**: any cap low enough to be safe is below what Babel needs, so it would turn 15 CI passes red. The cap exists for user scripts, and defaults to off for exactly this reason.
