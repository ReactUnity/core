...
`elasticity` above 0 lets the content be pulled past its end and spring back, unless
[`overscroll-behavior: none`](/reference/css/overscroll-behavior) has taken that away. `direction` fixes which
way the view scrolls, and `alwaysShow` keeps the named scrollbars from auto-hiding.

A view that has reached its end hands the rest of the scroll to the view above it;
[`overscroll-behavior`](/reference/css/overscroll-behavior) is how one keeps it. Scrolling to a position,
animating that scroll and resting on the items inside are
