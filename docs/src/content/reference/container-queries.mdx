...
`scroll-state` is accepted alongside any of these, as in `container-type: inline-size scroll-state`, so a stylesheet written for the web still parses; there are no scroll-state queries, so it decides nothing.
