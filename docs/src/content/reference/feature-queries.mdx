...
`@supports` must be a top-level rule. Writing it nested inside a style rule, which CSS nesting
allows, does not work — the bundled CSS parser drops the nested block while keeping the declarations
around it.
