...
- Inside a [`filter`](/reference/css/filter), that render is kept between frames and taken again only when something painted *before* the element has moved. An animation over a frosted panel therefore costs the panel nothing, while one behind it is read every frame.
