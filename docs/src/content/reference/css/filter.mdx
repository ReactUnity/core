...
What a capture costs is mostly the offscreen render itself rather than its size, so many filters on a page are captured together into one render instead of one each. Three things opt an element out of that and give it a render of its own: a `perspective`, a rotation, and a `backdrop-filter` or `mix-blend-mode` anywhere inside it.
