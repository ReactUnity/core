...
A nested `@container` works the way a nested `@media` does; see [container queries](/reference/rules/container-queries#nesting).
A nested `@scope` is different: only its start selector resolves against the enclosing rule, and the
rules inside it are relative to the scoping root; see [scoped styles](/reference/rules/scoped-styles#nesting).
