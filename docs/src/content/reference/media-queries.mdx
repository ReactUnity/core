...
### Boolean typed media properties {/*boolean-typed-media-properties*/}
...
### String typed media properties {/*string-typed-media-properties*/}
...
| `prefers-color-scheme` | `light` or `dark`                                                                                               | The preferred color scheme. Defaults to the editor theme in the editor and `light` elsewhere; the root element's `color-scheme` property seeds it, and `MediaProvider.SetValue` sets it outright. |
...
`hover`, `any-hover`, `pointer` and `any-pointer` also take the boolean form, which is true for any value but `none`. They are always defined, so `(hover)` is the standard way to gate hover-only styling and matches nothing on a touch-only device.
...
### Numeric media properties {/*numeric-media-properties*/}
...
`@custom-media --name query;` gives a query a name to use in place of a feature, so a condition
that recurs throughout a stylesheet is written once. The query is any media query list, or `true`
or `false`. A name is resolved when the query it is in is evaluated, so it may be declared in any
stylesheet, before or after its use, and the last declaration counts.

```css
@custom-media --gamepad (input: gamepad) or (any-input: joystick);
@custom-media --small (max-width: 600px);

@media (--gamepad) and (--small) {
  .hint { font-size: 18px; }
}
```
