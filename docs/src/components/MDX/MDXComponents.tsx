...
function CodeStep({ children, step }: { children: any; step: number }) {
  return (
    <span
      data-step={step}
      className={cn('code-step bg-opacity-10 dark:bg-opacity-20 relative rounded px-[6px] py-[1.5px] border-b-[2px] border-opacity-60', {
...
const P = (p: JSX.IntrinsicElements['p']) => <p className="whitespace-pre-wrap my-4" {...p} />;
...
const Wip = ({ children }: { children: React.ReactNode }) => <ExpandableCallout type="wip">{children}</ExpandableCallout>;
const Pitfall = ({ children }: { children: React.ReactNode }) => <ExpandableCallout type="pitfall">{children}</ExpandableCallout>;
const Deprecated = ({ children }: { children: React.ReactNode }) => <ExpandableCallout type="deprecated">{children}</ExpandableCallout>;
const Note = ({ children }: { children: React.ReactNode }) => <ExpandableCallout type="note">{children}</ExpandableCallout>;
    ...
    <>
      <section className="p-8 mt-16 mb-16 flex flex-row shadow-inner justify-between items-center bg-card dark:bg-card-dark rounded-lg">
        <div className="flex-col">
          <h2 className="text-primary dark:text-primary-dark font-bold text-2xl leading-tight">Ready to learn this topic?</h2>
          {children}
          {path ? (
...
function IllustrationBlock({
  ...
  const images = imageInfos.map((info, index) => (
    <figure key={index}>
      ...
      {info.caption ? (
        <figcaption className="text-secondary dark:text-secondary-dark text-center leading-tight mt-4">{info.caption}</figcaption>
      ) : null}
    </figure>
  ));
  return (
...
function calculateNestedToc(toc: Toc): NestedTocRoot {
  ...
  for (let i = startIndex; i < toc.length; i++) {
    const item = toc[i];
    const currentParent: NestedTocNode | NestedTocRoot = currentAncestors.get(item.depth - 1) || root;
