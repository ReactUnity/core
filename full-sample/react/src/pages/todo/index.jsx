...
export function TodoItem(props) {
  return (
    <view className="todo-item">
      ...
      <button className="todo-remove-button" onClick={() => props.onRemove(props.item.id)}>
        <icon>delete</icon>
      </button>
    </view>
  );
  ...
  return (
    <scroll className="todo-root">
      ...
      <view className="todo-input-section">
        <input className="todo-input" ref={inputRef} placeholder="Enter a new todo" onSubmit={(ev, sender) => addTodo(sender.Value)} />

        <button className="todo-add-button" onClick={() => addTodo(inputRef.current.Value)}>
          <icon>add</icon>
        </button>
      </view>
