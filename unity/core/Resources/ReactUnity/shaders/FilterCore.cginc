#ifndef REACTUNITY_FILTERCORE_INCLUDED
...
float4 frag(v2f i) : SV_Target
{
  ...
  // Skipped when neither op asks for anything. The round trip through HSV is only near-identity,
  // and `isolation: isolate` runs this whole chain with every value at its default -- an isolated
  // element has to come out of the capture as the pixels that went in.
  if (_HueRotate != 0 || _Saturate != 1)
  {
  ...
  // adds rather than covers, so its contribution has to be faded before it reaches the backdrop.
...
#ifdef RU_HAS_BACKDROP
  // `mix-blend-mode`. The whole filtered result -- drop shadow included -- is one group, which is
  // what CSS blends.
  if (_BlendMode != RU_BLEND_NORMAL && a > 0.0001)
  {
    ...
    {
      // Cs*as + Cb, clamped -- the source is added, not composited by coverage. The pass still
      // subtracts (1 - a) * Cb on its way out, so that is added back here to cancel it.
      rgb = min(1.0, rgb + backdrop) - (1.0 - a) * backdrop;
    }
    else rgb = RuBlend(_BlendMode, backdrop, saturate(rgb / a)) * a;
  }
#endif
