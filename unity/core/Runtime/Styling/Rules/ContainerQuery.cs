    ...
    public class ContainerQuery
    {
        ...
        public static string PrepareForParser(string css)
        {
            ...
            while (i < css.Length)
            {
                ...
                if (c == '@' && string.Compare(css, i, AtKeyword, 0, AtKeyword.Length, StringComparison.OrdinalIgnoreCase) == 0
                    && (i + AtKeyword.Length >= css.Length || !IsIdentChar(css[i + AtKeyword.Length])))
                {
        ...
        /// <summary>The prelude a media condition stands in for, when <see cref="PrepareForParser"/> put it there.</summary>
        public static bool TryDecodePrelude(string mediaCondition, out string prelude)
        {
            prelude = null;
            var text = mediaCondition?.Trim();
            if (text == null || !text.StartsWith(Marker, StringComparison.Ordinal)) return false;

            var encoded = text.Substring(Marker.Length).Replace('-', '+').Replace('_', '/');
            var padded = encoded.PadRight(encoded.Length + (4 - encoded.Length % 4) % 4, '=');

            try
            {
                prelude = Encoding.UTF8.GetString(Convert.FromBase64String(padded));
                return true;
            }
        ...
        // Base64 with the URL alphabet and no padding, so that the result is an identifier.
        private static string Encode(string prelude)
        {
            return Convert.ToBase64String(Encoding.UTF8.GetBytes(prelude)).TrimEnd('=').Replace('+', '-').Replace('/', '_');
        }
