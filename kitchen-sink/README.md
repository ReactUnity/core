...
- **The output folder name becomes the file name.** Unity writes `Build/<folder name>.wasm.unityweb`, not the product or profile name, so the folder has to be `WebInjectable` or `Previewer` — that is what both consumers request. The directory it is *published* into is `injectable`/`previewer`; only `Build/` is copied over (neither scene uses `StreamingAssets`, so no build produces one).
- **Compression is Gzip with `webGLDecompressionFallback` on**, which is what the `.unityweb` suffix on three of the four files means: the loader inflates them itself, because GitHub Pages cannot be told to send `Content-Encoding`. Both consumers ask for those names. The project is set that way, so nothing has to be flipped before a build — but flipping it produces file names neither consumer requests, and Unity says nothing. Off, the same player is ~82 MB rather than ~26 MB; the remaining bulk is a `Code Optimization` of `Shorter Build Time`.
- **The previewer's version is in its path**, so a new build means a new directory *and* the matching `PREVIEWER_VERSION` bump in `packages/scripts/config/public/index.html`. Upload first: a bump pointing at a directory that is not there yet gives every dev server a dead previewer. The injectable player has no version in its path, so uploading one *replaces* what the live docs site is loading — add the new files, let the docs deploy switch over, and only then delete what the previous build left behind.
