...
The parser drops a `@container` nested in a style rule and keeps a nested `@media` only when its prelude reads as a media query, so a stylesheet is rewritten before parsing: each `@container` block becomes a `@media` whose sole feature is the prelude encoded as one identifier, decoded again when the sheet is processed. Nothing about that is visible from CSS.
