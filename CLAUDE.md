...
- **TypeScript is pinned to 6.x workspace-wide, deliberately** — see the `//typescript` note in the root [package.json](package.json). TypeScript 7 is the native compiler and ships no JS API, which breaks `resolve.sync('typescript')` in [packages/scripts/config/modules.js](packages/scripts/config/modules.js) (so `react-unity-scripts` cannot start at all), fork-ts-checker behind it, and `astro check` in docs. It also removed `target: ES5`, which `renderer` and `material` emit for Unity's embedded JS engines — both tsconfigs carry `"ignoreDeprecations": "6.0"` to say so out loud. Three more deps are held back for reasons written next to them: `webpack-dev-server` (5.x — 6 needs Express 5, which the react-dev-utils middlewares predate), `@babel/core` (7.x — `babel-preset-react-app` peer-depends on it), and `react-unity-webgl` (9.x — `renderer/webgl-compat` re-exports its types as its own API).
