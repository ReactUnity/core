...
- TypeScript is 7.x everywhere except `docs`, which is on 6.x because `@astrojs/check` peer-depends on `^5 || ^6` and `astro check` needs the compiler's JS API, which 7 doesn't ship. Nothing may reintroduce `require('typescript')` or `resolve.sync('typescript')` — 7 has no CJS entry — and the options it removed (`target: ES5`, `esModuleInterop: false`, `baseUrl`) can't come back into a tsconfig. One dep is held back for the reason written next to it: `@babel/core` (7.x).
