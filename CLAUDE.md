...
- **The TypeScript version is split on purpose, so check the package you are editing.** The root, `packages/create`, `packages/renderer` and `packages/material` are on 7.x. Six are on 6.x, each carrying a `//typescript` note next to its own pin, with the summary in the root [package.json](package.json): [packages/scripts](packages/scripts/config/modules.js), the three `unity/core/.react/*` apps, `full-sample/react`, and `docs`. The blocker is that 7 is the native compiler and ships no JS API — its export map points `.` at `lib/version.cjs`, so `require('typescript')` yields `{ version, versionMajorMinor }` and `resolve.sync('typescript')` throws MODULE_NOT_FOUND. `scripts` calls that at three sites (`config/modules.js`, fork-ts-checker's `typescriptPath` in `config/webpack.config.js`, `scripts/utils/verifyTypeScriptSetup.js`), so `react-unity-scripts` cannot start at all — and the four apps go with it, since fork-ts-checker resolves TypeScript from the *app's* directory. Separately `docs` is blocked by `@astrojs/check`, which peer-depends on `^5.0.0 || ^6.0.0`. What used to block `renderer` and `material` was different and is now gone: 7 *removes* compiler options rather than deprecating them (TS5108 for `target=ES5` and `esModuleInterop=false`, TS5102 for `baseUrl`), and those were emit options — with tsdown owning emit and both tsconfigs `noEmit`, nothing reaches for them. Three more deps are held back for reasons written next to them: `webpack-dev-server` (5.x — 6 needs Express 5, which the react-dev-utils middlewares predate), `@babel/core` (7.x — `babel-preset-react-app` peer-depends on it), and `react-unity-webgl` (9.x — `renderer/webgl-compat` re-exports its types as its own API).
