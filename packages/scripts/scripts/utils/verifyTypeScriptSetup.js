...
function verifyTypeScriptSetup() {
  ...
  try {
    ...
    console.error(
      chalk.bold(
        `If you are not trying to use TypeScript, please remove the ${chalk.cyan('tsconfig.json')} file from your package root (and any TypeScript files).`,
      ),
    );
    console.error();
    process.exit(1);
  ...
  try {
    ...
    if (e && e.name === 'SyntaxError') {
      console.error(
        chalk.red.bold('Could not parse', `${chalk.cyan('tsconfig.json')}.`, 'Please make sure it contains syntactically correct JSON.'),
      );
    }
  ...
  for (const option of Object.keys(compilerOptions)) {
    ...
    if (suggested != null) {
      ...
      messages.push(
        ...
        )} be ${valueToCheck == null ? 'set' : chalk.cyan.bold(value)}${reason != null ? ` (${reason})` : ''}`,
      );
    }
  }
  ...
  if (!types.includes('@reactunity/scripts/main')) {
    ...
    messages.push(`${chalk.cyan('types')} should include ${chalk.cyan.bold('@reactunity/scripts/main')}`);
  }

  if (
    !['@reactunity/scripts/ugui', '@reactunity/scripts/uitoolkit', '@reactunity/scripts/editor', '@reactunity/scripts/tests'].includes(
      importSource,
    )
  ) {
