...
export function createGridComponent({
  ...
  return class Grid<T> extends PureComponent<Props<T>, State> {
    ...
    scrollToItem({
      ...
      const horizontalScrollbarSize = estimatedTotalWidth > width ? sizes.horizontalHeight : 0;
      const verticalScrollbarSize = estimatedTotalHeight > height ? sizes.verticalWidth : 0;

      this.scrollTo({
        scrollLeft:
          columnIndex !== undefined
            ? getOffsetForColumnAndAlignment(this.props, columnIndex, align, scrollLeft, this._instanceProps, verticalScrollbarSize)
            : scrollLeft,
        scrollTop:
          rowIndex !== undefined
            ? getOffsetForRowAndAlignment(this.props, rowIndex, align, scrollTop, this._instanceProps, horizontalScrollbarSize)
            : scrollTop,
      });
    }
    ...
    _getHorizontalRangeToRender(): [number, number, number, number] {
      const { columnCount, overscanColumnCount, rowCount } = this.props;
      const { horizontalScrollDirection, isScrolling, scrollLeft } = this.state;

      const overscanCountResolved: number = overscanColumnCount || 1;
    ...
    _getVerticalRangeToRender(): [number, number, number, number] {
      const { columnCount, overscanRowCount, rowCount } = this.props;
      const { isScrolling, verticalScrollDirection, scrollTop } = this.state;

      const overscanCountResolved: number = overscanRowCount || 1;
...
const validateSharedProps = ({ children, direction, height, width }: Props<any>, { instance }: State): void => {
